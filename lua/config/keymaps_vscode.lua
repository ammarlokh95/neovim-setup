...
map.set(
  "n",
  "<leader>wh",
  ":lua require('vscode').call('workbench.action.splitEditorDown')<CR>",
  { noremap = true, silent = true, desc = "Horizontal split" }
)
map.set(
  "n",
  "<C-h>",
  ":lua require('vscode').call('workbench.action.navigateLeft')<CR>",
  { noremap = true, silent = true, desc = "Move to left buffer" }
)
map.set(
  "n",
  "<C-j>",
  ":lua require('vscode').call('workbench.action.navigateDown')<CR>",
  { noremap = true, silent = true, desc = "Move to bottom buffer" }
)
map.set(
  "n",
  "<C-k>",
  ":lua require('vscode').call('workbench.action.navigateUp')<CR>",
  { noremap = true, silent = true, desc = "Move to top buffer" }
)
map.set(
  "n",
  "<C-l>",
  ":lua require('vscode').call('workbench.action.navigateRight')<CR>",
  { noremap = true, silent = true, desc = "Move to right buffer" }
)
map.set(
  "n",
  "<leader>we",
  ":lua require('vscode').call('workbench.action.evenEditorWidths')<CR>",
  { noremap = true, silent = true, desc = "Equalize splits" }
)
map.set(
  "n",
  "<leader>wx",
  ":lua require('vscode').call('workbench.action.closeWindow')<CR>",
  { noremap = true, silent = true, desc = "Close current splits" }
)
...
-- tabs
map.set("n", "<leader><tab>L", ":tablast<cr>", { desc = "Last Tab" })
map.set("n", "<leader><tab>o", ":tabonly<cr>", { desc = "Close Other Tabs" })
map.set("n", "<leader><tab>f", ":tabfirst<cr>", { desc = "First Tab" })
map.set("n", "<leader><tab><tab>", ":tabnew<cr>", { desc = "New Tab" })
map.set("n", "<leader><tab>l", ":tabnext<cr>", { desc = "Next Tab" })
map.set("n", "<leader><tab>d", ":tabclose<cr>", { desc = "Close Tab" })
map.set("n", "<leader><tab>h", ":tabprevious<cr>", { desc = "Previous Tab" })
map.set("n", "<leader>e", function()
  local vscode = require('vscode')
  vscode.call('workbench.action.toggleSidebarVisibility')
end, opts)

map.set("n", "<leader><leader>", "<cmd>lua require('vscode').action('find-it-faster.findFiles')<CR>", opts)
map.set("n", "<leader>bf", "<cmd>lua require('vscode').action('search-preview.showAllEditorsByMostRecentlyUsed')<CR>", opts)
map.set({"n", "v"}, "<leader>/", "<cmd>lua require('vscode').action('periscope.search')<CR>", opts)
